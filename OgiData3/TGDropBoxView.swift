...
let TGDropBoxListCellHeight: CGFloat = 50.0//35.0
let TGDropBoxBackgroundAlpha: CGFloat = 0.3
let TGDropBoxTitleMargin: CGFloat = 15
let TGDropBoxTitleFontSize: CGFloat = 16//14
let TGDropBoxTitleHoldFontSize: CGFloat = 13
let TGDropBoxListFontSize: CGFloat = 16//13
...
class TGDropBoxView: UIView {
    ...
    private let heightConstraint = ConstraintGroup()
    ...
    override func layoutSubviews() {
        ...
        */
        ...
        constrain(self.spaceLine) {
            spaceLine in
            spaceLine.left == spaceLine.superview!.left
            spaceLine.centerY == spaceLine.superview!.centerY
            spaceLine.width == 1
            spaceLine.height == spaceLine.superview!.height - 15
        }
        
        self.boxArrow.sizeToFit()
        //self.boxArrow.center = CGPoint(x: self.frame.size.width - 21, y: self.frame.size.height * 0.5)
        constrain(self.boxArrow) {
            boxArrow in
            boxArrow.center == boxArrow.superview!.center
        }
        
        /*
        constrain(self.listTableView, self) {
            table, sel in
            table.top == sel.bottom
            table.right == sel.right
            table.width == sel.width + 100.0
            table.height ==  CGFloat(self.items.count) * TGDropBoxListCellHeight
        }
        */
    }
    
    func setParentView(_ parentView : UIView) {
        constrain(self.listTableView, self, parentView, backgroundView) {
            table, sel, pv, bg in
            table.top <= sel.bottom
            table.right == sel.right
            table.left == pv.left
            table.bottom <= bg.bottom
        }
        constrain(self.listTableView, replace: heightConstraint) {
            table in
            table.height ==  min(10, CGFloat(self.items.count)) * TGDropBoxListCellHeight
        }
    }
    ...
    fileprivate func showBoxList() {
        ...
        UIView.animate(withDuration: TGDropBoxAnimateTime, animations: self.layoutIfNeeded, completion: { _ in
            self.didShowOrHideBoxListHandler?(true)
        })
    }
    ...
    fileprivate func rotateBoxArrow() {
        ...
        UIView.animate(withDuration: 0.2, animations: {
            self.boxArrow.transform = self.boxArrow.transform.rotated(by: angle * CGFloat(CGFloat.pi/180))
        })
    }
